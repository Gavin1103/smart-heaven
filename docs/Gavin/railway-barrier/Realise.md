# Railway Barrier System (ESP32 Servo + Ultrasonic)
...
The original design considered using two detection points (one for detecting an approaching train and one for detecting when the train has passed). However, the available kit only contains one ultrasonic sensor.

Because of this limitation, the implementation uses a single sensor for detecting if the train is approaching or has left

This approach reduces hardware complexity and fits within the constraints of the prototype, but it is less reliable than using multiple sensors.

In a future iteration, adding a second sensor would improve reliability and reduce ambiguity in train detection.